...
public class MmediaController {
    ...
    public int seek;//保存播放进度
    /*//为了实现随机播放
    private ImageView playMode;
    private Random ran=new Random();
    private List<String> listPath;
    private static int flat=0;// 0:表示顺序播放   1:表示单曲循环    2:表示随机播放*/
    ...
    private void initView() {
        ...
        //playMode=contentView.findViewById(R.id.playMode);
    }
    ...
    private void initListener() {
        ...
        requestOrien.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                seek = player.getDuration() * seekBar.getProgress() / 100;//保存转屏前播放进度
        ...
        pattern.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                int i = listPath.indexOf(uri.toString());
                listPath = player.getListPath();
                    if (5==player.mCurrentState){//表示某个播放完成,然后点击播放模式
                        if (2==flat){
                            pattern.setImageResource(R.mipmap.random);
                            int nextInt = ran.nextInt(listPath.size());
                            player.setVideoPath(listPath.get(nextInt));
                        }else if (0==flat){
                            player.setVideoPath(uri.toString());
                        }else {
                            ++i;
                            player.setVideoPath(listPath.get(i));
                        }
                        player.start();
                    }
            }
